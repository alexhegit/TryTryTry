...
def producer(cap, qf):
    while (True):
        ...
        if cv2.waitKey(1) & 0xFF == ord('q'):
            ...
            break
    print("Quit producer")
    print(" Producer: Frame Queue Size: %d" %(qf.qsize()))
...
def main(args):
    ...
    qf = queue.Queue(maxsize = 0)
    thread_list = []

    pt = Thread(target = producer, args = (cap, qf,))
    thread_list.append(pt)
    pt.start()

    for i in range(thread_number):
        ct = Thread(target = consumer, args = (qf, i, ))
        thread_list.append(ct)
        ct.start()


    print("Thread numbers = %d" %(len(thread_list)))
